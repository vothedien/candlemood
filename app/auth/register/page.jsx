...
export default function RegisterPage() {
  ...
  const handleSubmit = event => {
    ...
    if (!result?.success) {
      setError(result?.message || "Không thể đăng ký. Vui lòng thử lại.");
      return;
    }
        ...
        <Container>
          <div className="rounded-3xl border border-white/10 bg-zinc-900/60 p-6 sm:p-8">
            <div className="flex items-center gap-3 text-sm text-amber-200/70">
              ...
              <span>Tạo tài khoản để lưu mood yêu thích và thanh toán nhanh chóng.</span>
            </div>

            <h1 className="mt-6 text-2xl font-semibold text-amber-50">Đăng ký</h1>
            <p className="mt-2 text-sm text-amber-200/80">
              Đã có tài khoản? {" "}
              <Link href={`/auth/login?redirect=${encodeURIComponent(redirectTo)}`} className="text-amber-300 underline-offset-4 hover:underline">
                Đăng nhập
              </Link>
            </p>
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              <div className="space-y-2">
                <label htmlFor="name" className="text-sm font-medium text-amber-100/90">
                  Tên hiển thị
                </label>
                <input
                  id="name"
